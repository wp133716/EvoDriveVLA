...
AirSim VLA 部署推理脚本
======================
在 AirSim 仿真器中实时运行训练好的 VLA 模型，控制无人机飞行。
...
3. 构建 prompt（与训练格式一致）
4. 模型推理，输出 3D 未来轨迹
5. 解析 waypoints，发送运动指令
6. 循环执行

用法：
    python deploy_airsim_vla.py \
        --model_path /path/to/trained_model \
        --instruction "Fly a circle clockwise with radius 50m at 200m altitude with 8m/s speed" \
        --takeoff_altitude 200 \
        --max_steps 500 \
        --save_log flight_log.json \
        --save_image_dir ./camera_images
"""
...
CONTROL_HZ = 3          # 推理+控制频率 (Hz)
LOOKAHEAD_IDX = 5       # 使用第几个 waypoint 作为即时目标 (0-5)
...
def capture_images(client: airsim.MultirotorClient) -> dict:
    """采集 4 路相机图像，返回 {channel: PIL.Image}"""
    requests = [
...
def ego_to_world(local_x, local_y, local_z, ref_x, ref_y, ref_z, yaw_rad):
    ...
    world_x = ref_x + dx
    world_y = ref_y + dy
    world_z = ref_z - local_z  # 向上为正 → NED z 向下
...
def run_inference(
    ...
    # 1. 处理图像，获取 pixel_values 和 grid_thw（与训练代码一致）
    image_list = [images["CAM_FRONT"], images["CAM_LEFT"], images["CAM_RIGHT"], images["CAM_DOWN"]]
    pixel_values_list = []
    grid_thw_list = []
    ...
    for img in image_list:
        ...
        # 设置 size 参数，与训练代码一致 (data_qwen.py:217-218)
    ...
    # 2. 计算 grid_thw_merged（与训练代码一致）
    merge_size = processor.image_processor.merge_size
    grid_thw_merged = [
        int(g.prod().item() // (merge_size ** 2)) for g in grid_thw_list
    ]

    # 3. 将 <image> 替换为 vision tokens（与训练代码一致）
    # <|vision_start|> + <|image_pad|> * grid_thw_merged + <|vision_end|>
    ...
    # 4. 使用与训练完全相同的 chat_template (data_qwen.py:59)
    ...
    # # 将inputs写入本地文件，供调试使用，使用json格式，注意要将tensor转换为list,只保存前100个元素，并且保存一些统计信息（mean, std, min, max）
    # import json
    # inputs_to_save = {}
    ...
    #         v_cpu = v.cpu().numpy()
    #         inputs_to_save[k] = {
    #             "data": v_cpu.flatten().tolist(),
    #             "shape": v_cpu.shape,
    #             "dtype": str(v_cpu.dtype),
    #             "mean": float(v_cpu.mean()),
    #             "std": float(v_cpu.std()),
    #             "min": float(v_cpu.min()),
    #             "max": float(v_cpu.max()),
    #         }
    # with open("inputs_debug.json", "w") as f:
...
def parse_waypoints(text: str) -> List[Tuple[float, float, float]]:
    """解析模型输出的 3D waypoints 文本

    预期格式: [(x1,y1,z1), (x2,y2,z2), ...]
    """
...
def main():
    parser = argparse.ArgumentParser(description="AirSim VLA 部署推理")
    parser.add_argument("--model_path", type=str, required=True, help="训练好的模型路径")
    parser.add_argument("--instruction", type=str, default="Fly a circle clockwise with radius 50m at 298m altitude with 6.3m/s speed", help="飞行任务指令")
    parser.add_argument("--takeoff_altitude", type=float, default=298, help="起飞高度 (m)")
    parser.add_argument("--max_steps", type=int, default=500, help="最大推理步数")
    parser.add_argument("--speed", type=float, default=6.3, help="飞行速度 (m/s)")
    parser.add_argument("--device", type=str, default="cuda:0", help="推理设备")
    parser.add_argument("--visualize", action="store_true", help="显示相机画面")
    parser.add_argument("--save_log", type=str, default=None, help="保存飞行日志路径 (JSON)")
    parser.add_argument("--save_image_dir", type=str, default=None, help="保存相机图像目录")
    ...
    # --- 起飞 ---
    altitude = -args.takeoff_altitude  # NED: 向下为正
    ...
    # --- 初始化历史缓存 ---
    state_history = deque(maxlen=20)      # 原始状态缓存（高频）
    traj_history = deque(maxlen=HISTORY_LEN)  # 用于 prompt 的历史轨迹
    ego_history = deque(maxlen=4)         # 历史 ego 状态

    mission_goal = determine_mission_goal(args.instruction)
    print(f"Instruction: {args.instruction}")
    print(f"Mission Goal: {mission_goal}")
    print(f"控制频率: {CONTROL_HZ} Hz, 速度: {args.speed} m/s")
    ...
    for step in range(args.max_steps):
        ...
        # 更新历史轨迹（按 HISTORY_INTERVAL 间隔采样）
        ...
        # 4. 模型推理
        t0 = time.time()
        output_text = run_inference(model, processor, tokenizer, images, user_content, args.device)
        ...
        if not waypoints:
            print(f"  步骤 {step}: 无法解析 waypoints: {output_text[:100]}")
            time.sleep(control_interval)
            continue

        # 6. 选择目标点并转换到世界坐标
        wp_idx = min(LOOKAHEAD_IDX, len(waypoints) - 1)
        local_x, local_y, local_z = waypoints[wp_idx]
        yaw_rad = math.radians(current.yaw)
        ...
        if step % 1 == 0:
            print(
                ...
                f"目标 ({target_x:.1f}, {target_y:.1f}, {-target_z:.1f}m) | "
                f"推理 {infer_time:.2f}s | "
                f"WP[{wp_idx}] = ({local_x:.2f}, {local_y:.2f}, {local_z:.2f})"
            )
        ...
        if args.visualize and "CAM_FRONT" in images:
            ...
            # 绘制 waypoints 信息
            info = f"Step {step} | WP: ({local_x:.1f},{local_y:.1f},{local_z:.1f})"
            cv2.putText(front_cv, info, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            # cv2.imshow("VLA Drone - Front Camera", cv2.resize(front_cv, (960, 540)))
            if args.save_image_dir:
                os.makedirs(args.save_image_dir, exist_ok=True)
                cv2.imwrite(os.path.join(args.save_image_dir, f"vla_output_step_{step:03d}.jpg"), front_cv)
